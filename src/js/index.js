...
function newTask() {
    ...
    if (!input.value) {
        ...
        alert('Ja existe uma tarefa com essa mesma descrição')
    }
    else {
        let values = JSON.parse(localStorage.getItem(localStorageKey) || "[]")
        values.push({
...
function showValues() {
    ...
    for (let i = 0; i < values.length; i++) {
        list.innerHTML += `<li><span id="span-task">${values[i]['name']}</span><span id='btn-x' onclick='removeItem("${values[i]['name']}")'>❌</span></li>`
    }
}
